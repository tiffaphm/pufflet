...
function validatePanelGroupLayout({
  ...
  if (nextLayout.length !== panelConstraints.length) {
  ...
  } else if (!utils_compare.fuzzyNumbersEqual(nextLayoutTotalSize, 100)) {
    {
      console.warn(
    ...
    for (let index = 0; index < panelConstraints.length; index++) {
      ...
      utils_assert.assert(unsafeSize != null);
  ...
  if (!utils_compare.fuzzyNumbersEqual(remainingSize, 0)) {
    for (let index = 0; index < panelConstraints.length; index++) {
...
exports.validatePanelGroupLayout = validatePanelGroupLayout;
